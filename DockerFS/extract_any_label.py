...
def extract_structure(img_path, dest_path, structure):
    ...
    # Cargar los archivos .mgz
    left_img = nib.load(left_output_path)
    right_img = nib.load(right_output_path)
    # Calcular el volumen (número de voxels no nulos)
    left_volume = (left_img.get_fdata() > 0).sum()
    right_volume = (right_img.get_fdata() > 0).sum()
    print("La diferencia entre volúmenes es de: " + str(abs(left_volume - right_volume)))
