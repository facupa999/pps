...
if 'segmenting' not in st.session_state:
    st.session_state.segmenting = False
...
def segmentar(img_path, name_subject):
    st.session_state.segmenting = True
...
def iniciar_segmentacion(img_path,name_subject,uploaded_file):
    st.session_state.segmenting = True  # Desactiva el botón antes de ejecutar la función
    if not img_path=='' and name_subject:
...
def tab_1():
    ...
    st.button("Segmentar", on_click=iniciar_segmentacion,args=[img_path,name_subject,uploaded_file], disabled=st.session_state.segmenting)
